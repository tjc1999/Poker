...
var Table = function( id, name, eventEmitter, seatsCount, bigBlind, smallBlind, maxBuyIn, minBuyIn, privateTable, blindIncreases, blindInterval, minActionTimeout, maxActionTimeout ) {
	...
	this.public = {
		...
		maxBuyIn: maxBuyIn,
		// Order of blind increases
		blindIncreases: blindIncreases,
		// Which blind are we on
		currentBlind: 0,
...
Table.prototype.increaseBlind = function() {
	if(this.public.currentBlind>=this.public.blindIncreases.length) this.public.currentBlind = 0;
	if(this.public.currentBlind<this.public.blindIncreases.length){
	this.public.smallBlind = this.public.blindIncreases[this.public.currentBlind];
	}else{
